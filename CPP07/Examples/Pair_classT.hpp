/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   Pair_classT.hpp                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: esoulard <esoulard@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/12 12:10:00 by esoulard          #+#    #+#             */
/*   Updated: 2021/01/12 12:44:31 by esoulard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <iostream>
#include <iomanip>

/*----------------------------------------
*-----------------------------------------
*TEMPLATE AVEC 2 TYPES DE VARIABLES
*/

template< typename T, typename U>
class Pair {

	public:

		Pair<T, U>(T const & lhs, U const & rhs) : _lhs(lhs), _rhs(rhs) {
			std::cout << "Generic template" << std::endl;
			return ;
		};

		~Pair<T, U>(){};

		T const & fst(void) const {return this->_lhs;}
		U const & snd(void) const {return this->_rhs;}

		//[...etc]

	private:
		T const _lhs;
		U const _rhs;


		Pair<T, U>(void);
};

/*----------------------------------------
*-----------------------------------------
*	SPECIALISATION PARTIELLE D'UN TEMPLATE
*/

template< typename U>
class Pair <int, U>{
//il faut désormais préciser les types après la classe
	public:

		Pair<int, U>(int lhs, U const & rhs) : _lhs(lhs), _rhs(rhs) {
			std::cout << "Int partial specialization" << std::endl;
			return ;
		};

		~Pair<int, U>(){};

		int 		fst(void) const {return this->_lhs;}
		U const & 	snd(void) const {return this->_rhs;}

		//[...etc]

	private:
		int 	_lhs;
		U const &_rhs;


		Pair<int, U>(void);
};


/*----------------------------------------
*-----------------------------------------
*	SPECIALISATION COMPLETE D'UN TEMPLATE
*/

template<>
class Pair <bool, bool>{
//il faut désormais préciser les types après la classe
	public:

		//on stocke les 2 booléens dans 2 bits d'un meme int
		Pair<bool, bool>(bool lhs, bool rhs) {
			std::cout << "Bool/bool specialization" << std::endl;
			this->_n = 0;
			this->_n |= static_cast<int>(lhs) << 0;
			this->_n |= static_cast<int>(rhs) << 1;
			return ;
		};

		~Pair<bool, bool>(){};

		bool 	fst(void) const {return this->_n & 0x01;}
		bool 	snd(void) const {return this->_n & 0x02;}

		//[...etc]

	private:
		int 	_n;

		Pair<bool, bool>(void);
};

//------------------------------------
template <typename T, typename U>
std::ostream & operator<<(std::ostream &o, Pair <T, U> const &p) {

	o << "Pair( " << p.fst() << ", " << p.snd() << " )";
	return o;
}

std::ostream & operator<<(std::ostream &o, Pair <bool, bool> const &p) {

	o << std::boolalpha << "Pair( " << p.fst() << ", " << p.snd() << " )";
	return o;
}